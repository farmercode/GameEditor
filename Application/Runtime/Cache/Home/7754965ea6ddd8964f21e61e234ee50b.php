...
<script type="text/javascript" src="/index.php?m=Home&c=js&a=userPlatForm"></script>
<script src="<?php echo (RESOURCE_URL); ?>js/linkage.js"></script>
    ...
    <ul class="nav nav-pills nav-stacked nav-bracket">
      ...
      <a href=""><i class="fa fa-suitcase"></i> <span>副本掉落</span></a>
      	...
      	<ul class="children" style="display: block">
	        ...
	        <li><a href="<?php echo U('Loot/lootList');?>">          
	...
	<div class="headerbar">
      ...
      <form class="form-inline" action="" method="post">
             <div class="col-sm-12">
              ...
              <div class="row">
                        <div class="col-sm-4" style="min-width: 120px;">                
                           <select class="form-control chosen-select linkage" data-placeholder="选择">     
                           <option>1</option>             
                           </select>
                        </div>
                        <div class="col-sm-4" style="min-width: 120px;">                
                       ...
                       <div class="col-sm-4" style="min-width: 120px;">                
                        <select class="form-control chosen-select linkage" data-placeholder="选择" name="server">
                        <option>1</option>             
                        </select>
                       </div>
              </div>        
...
<script type="text/javascript">
<?php if(!empty($userPlatForm)): ?>var _selected_server = {"version":"{$userPlatForm.Version}","platform":"<?php echo ($userPlatForm["SiteCode"]); ?>","server":"<?php echo ($userPlatForm["ServerCode"]); ?>"};<?php endif; ?>
      var _select_classname = "linkage";
       ...
       $("select[name=server]").on("change",function(){
          ...
          console.log(_server);
          $.ajax({
            url : "<?php echo U("Ajax/choseServer");?>",
            type : "post",
            data : {"code":_server},
            success : function(){
      ...
      <div class="col-md-12">          
          <div class="panel panel-info">
            ...
            <div class="panel-body">
               <a class="btn btn-warning" href="<?php echo U("Manager/newSysMail");?>" data-toggle="modal" data-target=".bs-example-modal-panel">发送邮件</a>
            </div>
      ...
      <div class="col-md-12">
            <div class="table-responsive">
                <table class="table table-info mb30">
                    ...
                    <tbody>
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$mail): $mod = ($i % 2 );++$i;?><tr>
                        <td><?php echo ($mail["mail_id"]); ?></td>
                        <td><?php echo ($mail["server_code"]); ?></td>                         
                        <td><?php echo ($mail["CharName"]); ?></td>
                        <td><?php echo ($mail["Title"]); ?></td>
                        <td><?php echo ($mail["Content"]); ?></td>                         
                        <td><?php if($mail['Status'] == 1): ?>未发送<?php else: ?>已发送<?php endif; ?></td>  
                         <td><?php echo (date("Y-m-d H:i:s",$mail["CreateTime"])); ?></td>                                    
                        <td><a class="btn btn-lightblue small_btn" data-toggle="modal" data-target=".bs-example-modal-panel" href="<?php echo U("Manager/sendSysMail",array('mid'=>$mail['mail_id']));?>">发送</a>
                            <a class="btn btn-danger small_btn del_action" href="<?php echo U("Manager/delSysMail",array('mailID'=>$mail['mail_id']));?>">删除</a>
                        </td>
